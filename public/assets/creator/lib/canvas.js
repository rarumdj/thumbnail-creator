    ...
    try {
      ...
      if (savedCanvas) {
        ...
        if (explodedPath.length > 2) {
          // LOAD TEMPLATE
          templates.load(explodedPath[2], explodedPath[3] === undefined ? '' : explodedPath[3]).then(response => {
            let data = response.data;
            let jsonData = JSON.parse(data.jsonContent);
            if (jsonData) {
              let dimensions = data.dimension.split('x');
              let width = dimensions[0];
              let height = dimensions[1];

              $(`${this.containerSelector} .toolpanel#background-panel .content #input-width`).val(width);
              $(`${this.containerSelector} .toolpanel#background-panel .content #input-height`).val(height);

              fabricCanvas.setWidth(width)
              fabricCanvas.originalW = width
              fabricCanvas.setHeight(height)
              fabricCanvas.originalH = height
              fabricCanvas.renderAll()

              document.querySelector("#loading").style.visibility = "hidden";
              fabricCanvas.loadFromJSON(jsonData, fabricCanvas.renderAll.bind(fabricCanvas));
