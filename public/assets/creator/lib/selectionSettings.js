...
(function () {
  ...
  var selectionSettings = function () {
    const _self = this;
    $(`${this.containerSelector} .main-panel  .toolpanel-container`).append(`<div class="toolpanel" id="select-panel"><div class="content"><p class="title">Selection Settings</p></div></div>`);
    ...
    (() => {
      ...
      AlignmentButtonList.forEach(item => {
        buttons += `<button title="Align ${item.pos}" data-pos="${item.pos}">${item.icon}</button>`
      })
      $(`${this.containerSelector} .toolpanel#select-panel .content`).append(`
    ...
    (() => {
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .object-options #backgroundImage`).click(() => {
        if (this.activeSelection.type !== 'image') return;
        let templateUrl = this.canvas.getActiveObject()._originalElement.currentSrc;
    ...
    (() => {
      $(`${this.containerSelector} .toolpanel#select-panel .content`).append(`
        <div class="effect-section">
          ...
          <hr>
          <h5>Background</h5>
          <button id="remove_background" class="form-control">Remove Background</button>
          <hr>
        </div>
      `);

      $(`${this.containerSelector} .toolpanel#select-panel .content #remove_background`).click(function () {
        if (_self.activeSelection.type !== 'image') return;

        document.querySelector("#loading").style.visibility = "visible";
        let templateUrl = _self.canvas.getActiveObject()._originalElement.currentSrc;

        templates.removeBackground(templateUrl).then((response) => {
